...
### black
covar_onco_afr = fread("../PRS_yxzhang/train/black/pheno_532.txt")
load("trait_PRS_data/onco_black.RData")
prs = prs[match(covar_onco_afr$IID,rownames(prs)),];prs[,1:29] = -prs[,1:29]
prs_onco_afr = apply(prs,2,scale)

covar_NCI = fread("../PRS_yxzhang/validation/NCI/pheno_5578.txt")
load("trait_PRS_data/NCI_black.RData")
prs = prs[match(covar_NCI$IID,rownames(prs)),];prs[,1:29] = -prs[,1:29]
prs_NCI = apply(prs,2,scale)

## AOU
load("AOU/multi_trait_score.RData");load("AOU/LungCa_V3_afr.RData")
...
prs_aou = prs_aou[,match(colnames(prs_onco_afr),colnames(prs_aou))]

# tf = dta$ancestry_pred=="eas"
# summary(glm(lung_ca_combine~prs_aou[tf,"LungCa_V2_eas"],family = binomial,data = dta[tf,]))

tf = dta$ancestry_pred=="afr"
prs_aou1 = prs_aou[tf,]
prs_aou1 = scale(prs_aou1)
covar_aou1 = covar_aou[tf,]

# for(i in 1:ncol(prs_aou1)){
#     print(colnames(prs_aou1)[i])
#     print(summary(glm(lung_ca_combine~prs_aou1[,i],family = binomial,data =covar_aou1)))
#     print(roc(covar_aou1$lung_ca,prs_aou1[,i])$auc)
# }
...
temp = fread("LungCa_V3/Onco_TRICL_white_prs.txt")
prs[,35] = temp$PRS[match(covar_TRICL$individual_ID,temp$IID)]
prs = apply(prs,2,scale)
tf = covar_TRICL$ethnicity1==5
covar_TRICL1 = covar_TRICL[tf,]
prs_TRICL1 = prs[tf,]


y = c(covar_onco_afr$Lungcancer,covar_NCI$Lungcancer,covar_TRICL1$Lungcancer,covar_aou1$lung_ca_combine)
x = rbind(prs_onco_afr,prs_NCI,prs_TRICL1,prs_aou1)
x1 = x[,1:32]
label = c(rep("onco",nrow(prs_onco_afr)),rep("NCI",nrow(prs_NCI)),rep("TRICL",nrow(prs_TRICL1)),rep("aou",nrow(prs_aou1)))
...
# fit <- grpreg(x1, y, group, penalty = "grLasso",family="binomial",lambda = 0.002)
# fit_se = select(fit, "BIC")
...
write.csv(rlt,"catboost_rlt_black.csv",row.names=F,quote=F)
...
id = "NCI";roc(y[label==id],y0[label==id])$auc;ORSD(y[label==id],y0[label==id])
...
save(y,y0,y1,label,x1,x2,file = "trait_PRS_data/final_PRS_afr.RData")
